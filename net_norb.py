...
'''
Description: This script contains the encoder-decoder networks for the NORB dataset.
'''
...
class FeatMap(nn.Module):
    ...
    def __init__(self, p_f):
        ...
        self.fc1 = torch.nn.Linear(c * 4 * 12 * 12, p_f)
    ...
    def forward(self, x):
        ...
        x = x.reshape(x.size(0), c * 4 * 12 * 12)
        x = self.fc1(x)
        return x  # p_f
...
class PreImgMap(nn.Module):
    ...
    def __init__(self, p_f):
        ...
        self.fc1 = nn.Linear(in_features=p_f, out_features=c * 4 * 12 * 12)
    ...
    def forward(self, x):
        ...
        if x.dim() == 1:
            x = x.reshape(1, c * 4, 12, 12)
        else:
            x = x.reshape(x.size(0), c * 4, 12, 12)
        x = F.leaky_relu(self.conv3(x), negative_slope=0.2)
        x = F.leaky_relu(self.conv2(x), negative_slope=0.2)
        x = torch.sigmoid(self.conv1(x))
        return x  # N x 96 x 96 x 1
