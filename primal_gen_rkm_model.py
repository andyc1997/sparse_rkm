...
# logger
file_handler = logging.FileHandler(filename='model_PrimalGenRKM_{ct}.log'.format(ct=time.strftime('%Y%m%d-%H%M')))
...
# parser
parser = parser_gen_rkm(model='PrimalGenRKM')
...
folder = './{dataset}/N-{N}/score/{model}'.format(dataset=args.dataset, N=N, model='PrimalGenRKM')
dirs = CreateDirs(ct=ct, folder=folder, model='PrimalGenRKM')
dirs.create()
...
def primal_rkm_loss(phi, X):
    ...
    h = F.normalize(torch.mm(phi, U), dim=0)  # avoid dividing by singular values
