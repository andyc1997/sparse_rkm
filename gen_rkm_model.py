...
# logger
file_handler = logging.FileHandler(filename='model_GenRKM_{ct}.log'.format(ct=time.strftime('%Y%m%d-%H%M')))
...
folder = './{dataset}/N-{N}/score/{model}'.format(dataset=args.dataset, N=N, model='GenRKM')
dirs = CreateDirs(ct=ct, folder=folder, model='GenRKM')
dirs.create()
