...
from parsers import parser_ct_gen_rkm
from utils import save_score
from validators import checkargs_ct_gen_rkm
...
# logger
file_handler = logging.FileHandler(filename='model_CTPrimalGenRKM_{ct}.log'.format(ct=time.strftime('%Y%m%d-%H%M')))
...
# parser
parser = parser_ct_gen_rkm(model='CTPrimalGenRKM')
...
N, p = data.shape
li_loss = []
checkargs_ct_gen_rkm(args, data, args.batch_size)
logging.info('*'*30 + '\nModel: Covariance-thresholding Primal Gen-RKM' + '\nParameters: c_stab = {c_stab} and c_accu = {c_accu}\n'.format(c_stab=args.c_stab, c_accu=args.c_accu) + '*'*30)
...
folder = './{dataset}/N-{N}/score/{model}'.format(dataset=args.dataset, N=N, model='CTPrimalGenRKM')
dirs = CreateDirs(ct=ct, folder=folder, model='CTPrimalGenRKM')
dirs.create()
...
def mad(X):
    mu = torch.median(X)
    mad = torch.median(torch.abs(X - mu))
    return mad


# MAD along row-axis
def mad_axis(X):
    mu, _ = torch.median(X, dim=0)
    mad, _ = torch.median(torch.abs(X - mu), dim=0)
    return mad


# Covariance thresholding
def adaptive_ct(X, n, p, const, device):
    # data dependent tau
    nu = 0.6745
    ...
    # covariance matrix
    Sig = torch.mm(Xc.t(), Xc)/n - (noise**2)*torch.eye(p).to(device)

    # soft thresholding
    eta = F.softshrink(Sig, tau/n**0.5)
    ...
    # support estimation
    noise_epsq = mad_axis(v)/nu*const
    noise_epsq = noise_epsq.repeat(p, 1)
    W = torch.where(torch.abs(v) >= noise_epsq, v, 0.)
    W = F.normalize(W, dim=0)
    #s = torch.sum(W * torch.mm(Sig, W), dim=0) * n
    return W, n*s**2
...
def primal_rkm_loss(phi, X):
    ...
    U1, s1 = adaptive_ct(phi, phi.shape[0], phi.shape[1], args.const, device)
    U1 = U + U1.detach() - U.detach()  # Straight-through estimator
    s1 = s + s1.detach() - s.detach()  # Straight-through estimator

    W = torch.mm(U1, torch.diag(s1**0.5))  # = W in dual form
    h = F.normalize(torch.mm(phi, U1), dim=0)  # avoid dividing by singular values, thanks to renormalization
    Phi_hat = torch.mm(torch.mm(phi, U1), U1.t())  # projection to feature space
    ...
    f2 = 0.5 * torch.trace(torch.mm(h, torch.mm(torch.diag(s1), h.t())))  # h regularization
...
U1, s1 = adaptive_ct(phi, phi.shape[0], phi.shape[1], args.const, device)
h = F.normalize(torch.mm(phi, U1), dim=0)
